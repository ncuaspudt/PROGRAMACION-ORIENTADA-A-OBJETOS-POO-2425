...
def tarea_completada():
    ...
    if tarea_seleccionada:  # Verificamos que haya una tarea seleccionada
        ...
        # Añadir un formato visual (tachar la tarea)
...
def eliminar_tarea():
    try:
        ...
        pass  # Si no hay tarea seleccionada, no pasa nada

# Creación de la ventana principal
root = tk.Tk()
root.title('Aplicacion GUI de Lista de Tareas ')
...
lista = tk.Listbox(root, height=10, width= 40)
lista.pack(pady=20)
...
root.bind("<k>", lambda event: tarea_completada())
root.bind("<j>", lambda event: eliminar_tarea())
